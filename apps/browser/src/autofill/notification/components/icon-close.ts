...
export function IconClose({ theme }: { theme: NotificationBarIframeInitData["theme"] }) {
  const iconFillColor = (themes as any)[theme]?.textColor || "none";
  const iconPathStyles = css`
    fill: ${unsafeCSS(iconFillColor)};
  `;

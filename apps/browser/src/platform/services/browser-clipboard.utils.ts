...
class BrowserClipboardUtils {
  ...
  static async copy(globalContext: Window, text: string) {
    if (!BrowserClipboardUtils.isClipboardApiSupported(globalContext, "writeText")) {
    ...
    try {
      ...
      BrowserClipboardUtils.consoleLogService.debug(
  ...
  static async read(globalContext: Window): Promise<string> {
    if (!BrowserClipboardUtils.isClipboardApiSupported(globalContext, "readText")) {
  ...
  private static useLegacyCopyMethod(globalContext: Window, text: string) {
    if (!BrowserClipboardUtils.isLegacyClipboardMethodSupported(globalContext, "copy")) {
      BrowserClipboardUtils.consoleLogService.warning("Legacy copy method not supported");
      return;
    }
    ...
    try {
      ...
      BrowserClipboardUtils.consoleLogService.warning(`Error writing to clipboard: ${error}`);
    } finally {
  ...
  private static useLegacyReadMethod(globalContext: Window): string {
    if (!BrowserClipboardUtils.isLegacyClipboardMethodSupported(globalContext, "paste")) {
      BrowserClipboardUtils.consoleLogService.warning("Legacy paste method not supported");
      return "";
    }
    ...
    try {
      ...
      BrowserClipboardUtils.consoleLogService.warning(`Error reading from clipboard: ${error}`);
    } finally {
      globalContext.document.body.removeChild(textareaElement);
    }
...
export default BrowserClipboardUtils;
